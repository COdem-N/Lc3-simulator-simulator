...
int setCC (CPU_p cpu) {
	...
	if (cpu->main_bus == 0) cpu->psr |= ZERO_FLAG_MASK;
	sign_bit = (int) (cpu->main_bus & SIGN_BIT_MASK);
	if (sign_bit) cpu->psr |= NEG_FLAG_MASK;
	else cpu->psr |= POS_FLAG_MASK;
}
...
int controller (CPU_p cpu, Register mem[]) {
    ...
    while (flag == 0) {   // efficient endless loop
        switch (state) {
            ...
            case FETCH_OP: // Look at ST. Microstate 23 example of getting a value out of a register
               ...
               switch (opcode) {
					...
					case AND:
						...
						else cpu->alu->B = immed_offset;	// second operand
						break;
					case NOT:
						cpu->alu->A = cpu->reg_file[sr1];
						break;
...
int textgui(CPU_p cpu,  Register mem[])
{
    ...
    while (flag == 0)
    {
        ...
        while (i < 16)
        {
			move(4+i, 27);
            clrtoeol();
            mvprintw(4 + i, 28, "x%04X:", mem[0] + aMemLocation );
            mvprintw(4 + i, 35, "x%04X", mem[i+1]);
            aMemLocation++;
            i++;
        }
        ...
        if (str[0] == '1')
        {
            ...
            if (file)
            {
                ...
                move(23, 4);
                clrtoeol();
                mvprintw(23, 4, "File loading successful ending at x%X",mem[29]);
				cpu->pc = mem[0];
